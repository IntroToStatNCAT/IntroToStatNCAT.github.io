...
<meta property="og:title" content="Introduction to R and RStudio">
  ...
  <link rel="canonical" href="http://localhost:4000/files/Labs/InteractiveLabs/CL1_intro_to_r/CL1_intro_to_r.Rmd">
  <meta property="og:url" content="http://localhost:4000/files/Labs/InteractiveLabs/CL1_intro_to_r/CL1_intro_to_r.Rmd">
...
![](images/interface-sample.png){width=650px}
...
```{r install-packages, eval=FALSE, exercise = FALSE}
...
```{r Rcalcexamplefrac, exercise = TRUE}

```

```{r Rcalcexamplefrac-solution}
36 / (29 * (15 - 9))
```

```{r Rcalcexamplefrac-check}
grade_result(
  pass_if(~ identical(.result, 36 / (29 * (15 - 9))))
)
```
...
```{r exp-example, exercise = TRUE}
exp(1)
```

```{r log-example, exercise = TRUE}
log(1)
```

Now, we will test the property of $e^{ln(x)} = x = ln(e^x)$.

```{r logproperty-example, exercise = TRUE}
x = 10

exp(log(x))
```

```{r expproperty-example, exercise = TRUE}
x = 10
log(exp(x))
```
...
1. Use R to calculate $\frac{2.59 - \frac{22}{7}}{10-\sqrt{23}}$. Type your code in the code-chunk below and click run to show result.
...
<!-- (2.59 - 22/7)/(10 - sqrt(23)) -->
<!-- ``` -->

```{r testStatEval-check}
grade_result(
  pass_if(~ identical(.result, (2.59 - 22/7)/(10 - sqrt(23))))
)
```

2a. Find the natural log of $pi$ 

```{r testStatEval2a, exercise = TRUE}

```

<!-- ```{r testStatEval2a-solution} -->
<!-- log(pi) -->
<!-- ``` -->

```{r testStatEval2a-check}
grade_result(
  pass_if(~ identical(.result, log(pi)))
)
```

2b. Find the natural log of $e^{2\pi^2}$. 

```{r testStatEval2b, exercise = TRUE}

```

<!-- ```{r testStatEval2b-solution} -->
<!-- log(exp(2*pi^2)) -->
<!-- ``` -->

```{r testStatEval2b-check}
grade_result(
  pass_if(~ identical(.result, log(exp(2*pi^2))))
)
```

3a. Let $a=3$ and $b=7$. Check to see if you obtain the same values for $(a+b)^2$.

```{r testStatEval3a, exercise = TRUE}

```

<!-- ```{r testStatEval3a-solution} -->
<!-- a = 3 -->
<!-- b = 7 -->
...
```{r testStatEval3a-check}
grade_result(
  pass_if(~ identical(.result, (a+b)^2))
)
```

3b. Let $a=3$ and $b=7$. Check to see if you obtain the same values for $a^2 + 2ab + b^2$.

```{r testStatEval3b, exercise = TRUE}

```

<!-- ```{r testStatEval3b-solution} -->
<!-- a = 3 -->
<!-- b = 7 -->
...
```{r testStatEval3b-check}
grade_result(
  pass_if(~ identical(.result, a^2 + 2*a*b + b^2))
)
```

4. Let $x1 = [1,2,3,4,5]$ and $x2 = [6,9,1,11,5]$. Find $\frac{x1^2+x2}{2}$.

```{r testStatEval4, exercise = TRUE}

```

<!-- ```{r testStatEval4-solution} -->
<!-- x1 = c(1,2,3,4,5) -->
<!-- x2 = c(6,9,1,11,5) -->
...
```{r testStatEval4-check}
grade_result(
  pass_if(~ identical(.result, (x1^2 + x2)/2))
)
```

5. Let $x3 = [7,6,8,5,5,9,1]$ and $x4 = [1,2,3]$. Find $x3+x4$ and explain the reason behind obtaining this result.

```{r testStatEval5, exercise = TRUE}

```

<!-- ```{r testStatEval5-solution} -->
...
```{r testStatEval5-check}
grade_result(
  pass_if(~ identical(.result, x3 + x4))
)
```
...
```{r loadabrbuthnot-data, exercise = TRUE}
arbuthnot
```
...
```{r view-data, exercise = TRUE, eval=TRUE}
arbuthnot
```
...
Although we previously said that it is best practice to type all of your `R` code into the code chunk, it is better practice to type this command into your console. Generally, you should type all of the code that is necessary for your solution into the code chunk. 
...
6. What command would you use to extract just the counts of girls baptized? 

```{r testStatEval6, exercise = TRUE}

```

<!-- ```{r testStatEval6-solution} -->
<!-- arbuthnot$girls -->
<!-- ``` -->

```{r testStatEval6-check}
grade_result(
  pass_if(~identical(.result, arbuthnot$girls))
)
```
